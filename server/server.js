...
io.on('connection', socket => {
  ...
  socket.on('set_tecnicoID', (tecnicoID, cb) => {
    ...
    users.tecnicoID = socket.client.id
  ...
  socket.on('send_message', (payload, cb) => {
    ...
    io.to(users.to).emit('new_message', payload );
  ...
  socket.on('request_ride', (payload, cb) => {
    ...
    io.to(users.to).emit('ride_request', payload );
  ...
  socket.on('accept_ride', (payload, cb) => {
    ...
    io.to(users.to).emit('ride_confirmed', payload );
  ...
  socket.on('reject_ride', (payload, cb) => {
    ...
    io.to(users.to).emit('ride_rejected', payload );
  ...
  socket.on('request_phone', (payload, cb) => {
    ...
    io.to(users.to).emit('phone_request', payload );
  ...
  socket.on('accept_phone', (payload, cb) => {
    ...
    // fetch phone 
    // payload.phone
    io.to(users.to).emit('phone_confirmed', payload );
    io.to(users.from).emit('phone_confirmed', {from: payload.to, to:payload.from} );
